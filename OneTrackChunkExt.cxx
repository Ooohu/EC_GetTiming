...
void OneTrackChunkExt::CalculateTiming(Float_t XBeam, Float_t YBeam, Float_t dtRWMRawTime, Float_t dtBunchOffset, Float_t dt_corr)
{
	dtCorr = dt_corr;
	// Positions
	XDiff = X - XBeam;
...
void OneTrackChunkExt::CalculateGeometryAndEnergy(){
	...
	if (trackType==1) Mlep = Me;
		...
		MsgWarning("Warning: Unknown trackType, treated as electron");
		Mlep = Me;
	}
	Float_t Etot = Mlep + E;
   ...
   //CHECK, Keng, apply 1.02 correction here. now disable
   //EnuQERyan = 1.02*0.5 * ( 2.0 * Mn * Etot + Mp * Mp - Mn * Mn - Mlep * Mlep )/(Mn - Etot + cosTheta*sqrt( Etot*Etot - Mlep*Mlep ) );
   EnuQE     = EnuQERyan;
...
void OneTrackChunkExt::PrintValues(Float_t dtRWMRawTime, Float_t dtBunchOffset)
{
	// Energy
	MsgInfo(MsgLog::Form("%15s%15s%15s%15s%15s%15s\n","Evis", "UZ", "EnueQE", "Radius", "RtoWallB", "RtoWallF"));
	MsgInfo(MsgLog::Form("%15.2f%15.2f%15.2f%15.2f%15.2f%15.2f\n\n", E, UZ, EnuQE, R, RtoWallB, RtoWallF));
	
	// Position
	MsgInfo(MsgLog::Form("%15s%15s%15s%15s%15s%15s%15s\n","X", "Y","Z","UX", "UY", "UZ", "Track L"));
	MsgInfo(MsgLog::Form("%15.2f%15.2f%15.2f%15.2f%15.2f%15.2f%15.2f\n\n", X, Y, Z, UX, UY, UZ, 2.0 * distToMeanCer));	
